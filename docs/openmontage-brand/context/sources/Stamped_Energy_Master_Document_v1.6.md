...
description: "Stamped Intelligence — real-time load and equipment decisions for Indian manufacturers: ML baselines, agentic prescriptions with ₹ impact, verified with evidence (DISCOM bill optional), two pillars + shared plant context."
...
> **Canonical index:** For the full repository map and “where to find what,” see [`README.md`](/README.md) at repo root. This document is the **product & company** source of truth for GTM and research; engineering SSOT lives in [`external/technical/STAMPED_ARCHITECTURE.md`](/external/technical/STAMPED_ARCHITECTURE.md) (platform pack v2026.08.01, ADR-024–026).
...
**Company Name:** Stamped Energy (product: **Stamped Intelligence** — one product, not three SKUs)

**One-Line Position:**
A **read-only operational decision layer** for **energy-intensive Indian manufacturers** — **managing load and equipment in real time** via ML baselines and an **agentic prescription layer**, turning plant data into **assigned actions with ₹ impact**, **verified with evidence** from Stamped’s tracking stack (DISCOM bill confirmation optional), without replacing EMS or Industry 4.0 stacks, without PLC control writes, and without enterprise procurement cycles.

**Client narrative (canonical):** Four-step story for WhatsApp, decks, and discovery — (1) load & energy management in real time, (2) equipment ML on plant baselines, (3) prescriptions with verify loop, (4) agentic layer alongside. Full templates and I4.0 gap framing: [Client Positioning & Narrative v1](/core-product/Stamped_Client_Positioning_and_Narrative_v1.md).
...
**GTM category (August 2026):**
**Verified-with-evidence operational decision layer** — real-time load & energy management + equipment intelligence (ML on plant baselines) + agentic prescriptions, not an EMS replacement, not an ESG/carbon accounting platform, **not MES/CMMS/plant OS**. Enemy positioning: **insight without closure** (Industry 4.0 monitoring, dashboards, and audits that never become context-aware decisions with assigned actions and evidence-backed ₹).

**Industry 4.0 complement (client-facing):** Typical I4.0 stacks monitor and alert; Stamped adds **what to do next**, feasibility against production, **assigned action**, and **verified outcomes** — read-only on top of existing OT/EMS.
...
A software-first energy decision and optimization platform that integrates with a manufacturing plant's existing infrastructure — SCADA systems, PLCs, CNCs, energy meters, and production data — to identify specific energy inefficiencies, prescribe exact corrective actions with rupee-denominated impact, track execution, and verify savings **with evidence** from Stamped’s own telemetry and calculation stack (DISCOM bill confirmation optional).

**What Stamped Energy is not:**
A monitoring dashboard. An ESG reporting tool. A sustainability compliance platform. A hardware company. A PLC control system. An **MES**. A **CMMS**. A separate OEE / plant-OS product. Stamped Energy does not surface data — it closes the loop from insight to **assigned action** to **verified savings with evidence** (energy **efficiency** hero; plant **effectiveness** co-benefits on management Rx).

**Dual outcome (enterprise messaging):** Primary hero remains **energy efficiency — ₹ / SEC / bill lines** verified with evidence. **Plant effectiveness** (OEE / order-on-time / downtime risk) appears as **co-benefits on management prescriptions** when order context exists — **not** a third pillar or MES product. Secondary column — **operational sustainability** (plant-level intensity evidence, audit-ready M&V) — supports listed/global subsidiaries without rebranding as an ESG platform. See `marketing/content-strategy/website/stamped-website-sustainability-positioning-strategy.md`.
...
They will pay for a software-first layer that connects to what they already have, tells them exactly where the money is going in rupees per month, turns that insight into specific assigned actions, and verifies outcomes with evidence from Stamped’s tracking stack (DISCOM bill confirmation optional).
...
> Insight is only valuable if it reliably causes action. Action is only credible if it is measured. Measurement is only trusted if it is **verified with evidence** — in rupees on Stamped’s ledger (DISCOM bill confirmation optional).
...
**White space statement:** Hundreds of **large, independent** North India plants have the data infrastructure and the bill pain, but no prescriptive layer that closes the loop with **evidence-verified ₹** at a **60-day plant pilot** cadence — without a conglomerate IT procurement cycle.
...
6. **Verifies outcomes with evidence** — after the action is marked complete, Stamped tracks the relevant loads/meters, clears the operational fault on telemetry, and builds a potential vs realised ₹ ledger (DISCOM bill can confirm later)
7. **Improves** — learns from acted vs ignored prescriptions and verified outcomes to tighten baselines and ranking (human-in-the-loop)

The platform delivers **measurable, verified cost reduction** — not software features.

### 6.1 Technical framing on calls (not a category change)

**Outward category:** evidence-verified energy decision layer — energy cost savings, energy efficiency, sustainability evidence as by-product. Primary proof is **verified with evidence** from our stack; bill confirmation is secondary.

**On technical calls and decks**, Stamped Intelligence is explained in **exactly two** sections (same data plane, same loop) plus **shared plant context** (orders / departments / trade-offs / negotiation / Improve — **not** a third pillar, **not** an MES). Working canon: [Two-Pillar Technical Framing v1](/core-product/Stamped_Two_Pillar_Technical_Framing_v1.md) · platform [ADR-026](/external/decisions/024-026/ADR-026-two-pillars-shared-context.md) · [ADR-024](/external/decisions/024-026/ADR-024-holistic-plant-decisions.md) · [ADR-025](/external/decisions/024-026/ADR-025-improve-loop-step-06.md).

| Section | Name | Primary outcome |
| --- | --- | --- |
| Pillar 1 | **Load & Energy Efficiency Intelligence** | Maximum demand, time-of-day, tariff, idle/waste, thermal timing → **₹ energy efficiency** hero, verified with evidence |
| Pillar 2 | **Prescriptive Equipment Intelligence** | Early warning + maintenance-executable actions from electrical/process signatures (not vibration predictive maintenance) |
| Shared context | Orders, departments, trade-offs, negotiation, Improve | Makes management Rx practical; **effectiveness** (OEE / order / downtime) as co-benefits — not Pillar 3 |

Grey-zone: pure energy actions → Pillar 1; routine maintenance that also saves energy → Pillar 2. Commercial wedge remains the **60-Day Proof Run** (Pillar 1 evidence proof primary). Do **not** position Stamped as MES, CMMS, or a separate OEE product.
...
Canonical six-step loop (platform ADR-025):
...
| Step | What happens |
| --- | --- |
| **Connect** | SCADA / PLC / CNC / meters / bills / optional ERP-MES orders |
| **Observe** | Baselines, SEC, anomalies, equipment health signals |
| **Decide** | Ranked prescriptions with ₹ impact (Pillar 1 and/or Pillar 2 engines) |
| **Execute** | Assigned owner, WhatsApp + dashboard, optional negotiation revise |
| **Verify** | Ops-cleared / calculated M&V → potential vs realised ledger; bill path optional |
| **Improve** | Calibration + preference signals from follow/ignore outcomes (human-gated) |

**Savings engineering thesis:** 15–20% verified savings is **Σ (closed prescriptions across waste categories) × closure rate**, defended by evidence-backed M&V — not a single model score. Detection without closure is not the product.
...
Target range: Identify and begin eliminating 10–20% of non-production-linked energy consumption within 60 days. `[~]`
...
- **Purpose:** One evidence-verified savings line from Stamped tracking (ops-cleared), or clear kill criteria at Day 60; DISCOM bill confirmation optional
- **Duration:** ~60 days (connect → prescribe → assign → verify)
...
| **Reframes (outbound)** | **Margin Defense** (OEM price-down years) + **Evidence Referee** (trust/conversion) |
| **LinkedIn** | AIDA-structured posts — ICP + general audience; see `marketing/content-strategy/social/linkedin-posts.md` |
| **Strategic marketing audit** | `marketing/content-strategy/positioning/stamped-energy-rory-sutherland-marketing-interrogation-2026-06.md` |

### 10.4 Website & content

| Asset | Location |
| --- | --- |
| Content roadmap (SEO/GEO/AEO) | `marketing/content-strategy/website/stamped-content-roadmap.md` |
| Sustainability + Industry 4.0 (second column) | `marketing/content-strategy/website/stamped-website-sustainability-positioning-strategy.md` |
...
The entry offer exists for one purpose: **remove career risk from the first purchase and generate the first evidence-verified savings line** (ops-cleared on Stamped tracking; DISCOM bill can confirm later).
...
- **Goal:** Deliver at least one **evidence-verified ₹ saving** (ops-cleared ledger). Use that number to convert to subscription or multi-site expansion.
...
Win on **60-day plant proof** before incumbents' enterprise cycles. Cross-plant benchmarking becomes the moat after first logos.
...
| Document | Purpose |
| --- | --- |
| **`Stamped_Energy_Master_Document_v1.4.md`** | **This file** — identity, ICP summary, GTM, revenue, bets, north star |
...
| `marketing/content-strategy/social/linkedin-posts.md` | LinkedIn post bank (AIDA) |
| `marketing/content-strategy/positioning/stamped-energy-rory-sutherland-marketing-interrogation-2026-06.md` | Positioning strategy audit |
| `marketing/content-strategy/website/stamped-website-sustainability-positioning-strategy.md` | Website sustainability / Industry 4.0 |
| `marketing/content-strategy/website/stamped-content-roadmap.md` | SEO/GEO/AEO content roadmap |
